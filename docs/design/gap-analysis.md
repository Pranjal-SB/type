...
| 17 | **Being fixed in M2.5, not yet released.** ~~**Theming is hardcoded.** `ThemeColors::default()` is constructed inline in `App::new`.~~ A theme is a TOML file with a named `[palette]` and a typed `[ui]` table, parsed in `typ-core` and loaded by name from the config directory or from the binary, degraded to the terminal's colour depth at load. `ThemeColors` is 25 typed slots and the default is now the fallback for when no theme loads rather than the definition of the theme. Format and rubric documented in [`themes.md`](themes.md). **The other half of the row was decided against**: `typ-ui` and `typ-config` were not built and will not be — the seam falls elsewhere, see `architecture.md` §5. **Still open until v0.2.5 ships**: one theme is embedded, five more are in progress, and the rubric they are tuned against is itself defective — see #42. |
...
| **Themes** | many | **38, custom TOML** | many | few | **TOML + rubric, 1 shipped, 5 in progress** |
