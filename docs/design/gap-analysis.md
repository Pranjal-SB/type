...
| 46 | LOW | **Markdown's inline grammar produces no highlights when injected.** Markdown ships as two grammars: `tree_sitter_md::LANGUAGE` for blocks and `INLINE_LANGUAGE` for the inside of a paragraph, the second reached through `((inline) @injection.content (#set! injection.language "markdown_inline"))`. Everything up to the parse is verified working: the marker resolves (`markdown_inline` → `Language::MarkdownInline` → `Language(4)`, confirmed by instrumenting `language_for_marker`), `get_config` returns the inline config, and that same config produces six spans on `Some *em* here.` when used as a **root** layer. As an injection it yields nothing — the layer is entered (event boundaries appear at exactly the inline node edges) but every span comes back with an empty highlight stack, which is what `start_highlight` does when a capture's `highlight_indices` entry is `None` or the layer has no tree. The likely cause is the inline grammar's external scanner over `set_included_ranges`. **Not a blocker for the language:** block structure (headings, fences, delimiters) works, and so does injection *out* of markdown — a fenced ```rust block is highlighted as Rust, which is the case injections were adopted for. What is missing is emphasis, links and inline code inside a paragraph. Helix runs the same crates through the same highlighter, so a working configuration exists to compare against. | `typ-syntax/src/language.rs` `Language::MarkdownInline`, `tree-sitter-md` 0.5.3 | unowned |
| 45 | LOW | **`app.rs` has grown a second responsibility, and it is search and replace.** 907 lines at M2.7 Task 4, past invariant 9's trigger. Looked, as the invariant requires, and the seam is not where M2.7's plan guessed: the parse-worker ownership that task added is ~45 lines that mirror the `rewatch`/`sender` pattern already there for the file watcher, so splitting it would make a module cohering around "things that talk to a worker" while leaving the watcher behind. The real group is `handle_prompt_chord`, `run_search`, `jump_to_match` and `run_replace_all` — **156 contiguous lines** of search/replace behaviour sitting beside a `prompt.rs` that owns the prompt's *state* and none of its *behaviour*, with `last_query` as the field that belongs with them. Predates M2.7 and is not that milestone's to fix; the split wants its own commit so the plan keeps matching the tree. | `typ-app/src/app.rs` `handle_prompt_chord`–`run_replace_all`, `typ-app/src/prompt.rs` | unowned |
